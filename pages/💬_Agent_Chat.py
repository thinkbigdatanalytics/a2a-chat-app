...
from agents import Agent, set_default_openai_client, ItemHelpers, OpenAIChatCompletionsModel
from reportlab.lib.pagesizes import letter
from reportlab.pdfgen import canvas

from Mathematical.MCP.math_toolsets import TimeoutMCPToolset
from db import get_all_agents, get_all_tools_by_id, Agents, get_session
...
def initiate_toolset(tool_name: str, command: str, timeout: int = 3000, **kwargs) -> NamedMCPToolset:
    if command == "npx":
    ...
    print(f"[DEBUG] Initiating MCP toolset '{tool_name}' with command '{command}' and args {kwargs}")
    return NamedMCPToolset(name=tool_name, connection_params=conn_params)

def load_agent_from_db(agent_id: int):
    with get_session() as session:
        row = session.query(Agents).filter(Agents.id == agent_id).first()
        if not row:
            return None
        ...
        if row.api_key and row.provider == "Google":
            os.environ["GOOGLE_API_KEY"] = row.api_key
        if row.api_key and row.provider == "azure_ai":
            os.environ["AZURE_API_KEY"] = row.api_key
        agent = Agent(
            name=row.name,
...
# Load sub-agents
sub_agents = [load_agent_from_db(aid) for aid in selected_agent_ids if load_agent_from_db(aid)]
...
main_agent = Agent(
    ...
    - Always reply in proper grammar, using full sentences.
    - Keep responses short and generic, like "The answer is 14."
